...
- Rationale: Fast (5ms), cheap ($0.02 per 1M tokens), good quality for semantic search
- Alternative: `all-MiniLM-L6-v2` (local, free, slightly worse quality)
...
- Model: **OpenAI GPT-4-turbo** (or Claude 3.5 Sonnet)
- Rationale: Best at following instructions + citations. GPT-4 > cheaper models for accuracy.
...
class LLMResponse(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "answer": "Based on Enterprise_SLA.pdf, the SLA for Enterprise customers is: 4-hour first response for P1 tickets, 24-hour for P2. See attached document for details.",
...
|-----------|--------|-----------|
| **Embedding Model** | `text-embedding-3-small` (OpenAI) | Fast (5ms), cheap, good quality. Alternative: `all-MiniLM-L6-v2` (local, free). |
| **LLM** | `gpt-4-turbo` (OpenAI) | Best at reasoning + citations. Can also use `claude-3.5-sonnet`. |
...
| **Chunk Size** | 512 tokens | Balances: enough context to answer most questions + precise retrieval. |
